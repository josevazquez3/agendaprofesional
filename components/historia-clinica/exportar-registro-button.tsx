...
interface ExportarRegistroButtonProps {
  pacienteId: string
  registroId: string
...
export function ExportarRegistroButton({
  pacienteId,
  registroId,
  pacienteNombre,
  nombreProfesional,
}: ExportarRegistroButtonProps) {
  ...
  const descargar = async (formato: "pdf" | "docx") => {
    ...
    try {
      const url =
        formato === "pdf"
          ? `/api/historia-clinica/exportar/pdf?pacienteId=${pacienteId}&registroId=${registroId}`
          : `/api/historia-clinica/exportar/doc?pacienteId=${pacienteId}&registroId=${registroId}`
      ...
      a.href = downloadUrl
      const base = [pacienteNombre || pacienteId, nombreProfesional].filter(Boolean).join("_").replace(/\s/g, "_")
      a.download = `historia_clinica_${base}.${formato === "pdf" ? "pdf" : "docx"}`
