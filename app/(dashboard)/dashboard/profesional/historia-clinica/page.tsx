...
export default function HistoriaClinicaProfesionalPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Pacientes con Historia Clínica</CardTitle>
          <CardDescription>
            Total: {pacientesFiltrados.length} paciente(s) con historias clínicas
          </CardDescription>
        </CardHeader>
        <CardContent>
          {pacientesFiltrados.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              {busqueda ? "No se encontraron pacientes" : "No hay pacientes atendidos"}
            </div>
          ) : (
              ...
              {pacientesFiltrados.map((paciente) => (
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <Link href={`/dashboard/profesional/historia-clinica/${paciente.id}`}>
                      <Button variant="outline" size="sm">
                        <Eye className="h-4 w-4 mr-2" />
                        Ver
                      </Button>
                    </Link>
                    <Link href={`/dashboard/profesional/historia-clinica/${paciente.id}/editar`}>
                      <Button variant="outline" size="sm">
